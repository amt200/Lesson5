...
const Getaboatapp = () => {
  for (let i = 0; i < PRODUCTS_DATA.length; i++) {
      var boat = PRODUCTS_DATA[i];
      boat_records.push(<Text key={boat.name}>{boat.name}</Text>);
      boat_records.push(<Text key={boat.description}>{boat.description}</Text>);
      boat_records.push(<Image source={boat.picture}/>);
  }
    return(
    <ScrollView>
        <View>{boat_records}</View>
    </ScrollView>
    );
};
